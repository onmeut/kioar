...
--
--   3. The composite index `(status, plan_key)` uses a regular (non-CONCURRENTLY)
--      build because drizzle-kit migrate runs inside a transaction block, which
--      Postgres forbids for CONCURRENTLY index builds.
...
-- NOTE: CONCURRENTLY is omitted here because drizzle-kit migrate runs
-- each migration inside a transaction block, which Postgres forbids for
-- CONCURRENTLY index builds.  The table is small enough at migration time
-- that a brief AccessShareLock is acceptable.
